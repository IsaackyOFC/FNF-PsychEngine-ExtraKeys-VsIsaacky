...
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuff:Array<Dynamic> = [
		['Try in easy', 0.2], //From 0% to 19%
		['omg', 0.4], //From 20% to 39%
		['very bad', 0.5], //From 40% to 49%
		['Try again', 0.6], //From 50% to 59%
		['Bad', 0.69], //From 60% to 68%
		['Meh', 0.7], //69%
		['Bah', 0.8], //From 70% to 79%
		['Good..', 0.9], //From 80% to 89%
		['Nice', 1], //From 90% to 99%
		['Very Nice', 1] //The value on this one isn't used actually, since Perfect is always "1"
	];
	...
	override public function create()
	{
		...
		botplayTxt = new FlxText(400, timeBarBG.y + 55, FlxG.width - 800, "You are a hacker, very cool", 32);
	...
	override public function update(elapsed:Float)
	{
		...
		if(ratingName == '?') {
			scoreTxt.text = 'Score: ' + songScore + ' | Note Errors: ' + songMisses + ' | Note hits: ' + ratingName;
		} else {
			scoreTxt.text = 'Score: ' + songScore + ' | Note Errors: ' + songMisses + ' | Note hits: ' + ratingName + ' (' + Highscore.floorDecimal(ratingPercent * 100, 2) + '%)' + ' - ' + ratingFC;//peeps wanted no integer rating
		}
